...
import { ClubResponse, EventResponse } from '../../core/models/api.models';
...
@Component({
  ...
  template: `
    ...
    @if (auth.hasRole('ClubManager')) {
      <p>
        <a routerLink="/event-manage">Yeni Etkinlik Oluştur</a>
      </p>
    }
    ...
    <button type="button" (click)="applyFilters()">Filtrele</button>
    <button type="button" (click)="clearFilters()">Filtreleri Temizle</button>
    <button type="button" (click)="loadEvents()">Yenile</button>
    ...
    @if (!loading() && events().length === 0 && !errorMessage()) {
      <p>Arama kriterlerine uygun etkinlik bulunamadı.</p>
    }

    @if (events().length > 0) {
      <p>Toplam etkinlik: {{ totalCount() }}</p>
      ...
      <table>
        ...
        <tbody>
          @for (event of events(); track event.id) { <!-- etkinlikleri tabloya basar -->
            <tr>
              <td>{{ event.id }}</td>
              <td>{{ event.title }}</td>
              ...
              <td>{{ event.status }}</td>
              <td>
                <a [routerLink]="['/events', event.id]">Detay</a>

                @if (ownsEvent(event)) { <!-- giriş yapan manager etkinliğin kulübünü yönetiyorsa yönetim işlemlerini gösterir -->
                  |
                  <a [routerLink]="['/event-manage', event.id]">Güncelle</a>
                  |
                  <a [routerLink]="['/events', event.id, 'registrations']">Kayıtlar</a>
                }
              </td>
            </tr>
          }
...
export class Events implements OnInit {
  ...
  readonly events = signal<EventResponse[]>([]); // backendden gelen etkinlikleri tutar
  readonly clubs = signal<ClubResponse[]>([]); // filtreleme ve yetki kontrolünde kullanılacak kulüpleri tutar
  readonly searchText = signal(''); // etkinlik arama metnini tutar
  ...
  readonly upcomingOnly = signal(false); // sadece yaklaşan aktif etkinliklerin gösterilip gösterilmeyeceğini tutar
  readonly page = signal(1); // aktif sayfa numarasını tutar
  ...
  readonly loading = signal(false); // etkinlikler yüklenirken işlemin devam edip etmediğini tutar
  readonly errorMessage = signal(''); // kullanıcıya gösterilecek hata mesajını tutar

  ngOnInit(): void { // sayfa açıldığında kulüpleri ve etkinlikleri getirir
    if (this.auth.hasRole('Student')) {
      this.upcomingOnly.set(true); // Student için varsayılan olarak sadece yaklaşan aktif etkinlikleri gösterir
    }
  ...
  ownsEvent(event: EventResponse): boolean { // giriş yapan managerın etkinliğin bağlı olduğu kulübü yönetip yönetmediğini kontrol eder
  ...
  loadClubs(): void { // kulüp filtresi ve yetki kontrolü için tüm kulüpleri backendden getirir
  ...
  loadEvents(): void { // filtre ve sayfalama bilgilerine göre etkinlikleri backendden getirir
    ...
    this.eventService.getPaged({
      ...
      error: (error: HttpErrorResponse) => {
        ...
        this.errorMessage.set(
          getApiErrorMessage(error, 'Etkinlikler alınamadı.')
        );
        this.loading.set(false);
      }
    });
  ...
  applyFilters(): void { // seçilen filtreleri ilk sayfadan başlayarak uygular
    this.page.set(1);
    this.loadEvents();
  }

  clearFilters(): void { // tüm filtreleri temizleyip etkinlikleri tekrar getirir
    this.searchText.set('');
    this.category.set('');
  ...
  private getDateFrom(): string | undefined { // seçilen başlangıç tarihini backendin kullanacağı formata çevirir
  ...
  private getDateTo(): string | undefined { // seçilen bitiş tarihini günün sonuna ayarlayıp backendin kullanacağı formata çevirir
